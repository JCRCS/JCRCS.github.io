...
if (function_exists('vc_map')) {
    vc_map(array(
        "name" => __("ARS Rental Map Single", ST_TEXTDOMAIN),
        "base" => "ars-rental-map-single",
...
if (!function_exists('ars_vc_rental_map_single')) {

    function ars_vc_rental_map_single($attr, $content = false) {
        if (is_singular('st_rental')) {
            $template = ARS_THEME_DIR . "/st_templates/rentals/elements/map-single.php";
...
if (st_check_service_available('st_rental') AND function_exists('st_reg_shortcode')) {
    st_reg_shortcode('ars-rental-map-single', 'ars_vc_rental_map_single');
}