...
if (function_exists('vc_map')) {
    vc_map(array(
        "name" => __("ARS Rental Price Book", ST_TEXTDOMAIN),
        "base" => "ars_rental_price_book",
...
if (!function_exists('ars_vc_rental_price_book')) {

    function ars_vc_rental_price_book($attr, $content = false) {
        if (is_singular('st_rental')) {
            $template = ARS_THEME_DIR . "/st_templates/rentals/elements/price_book.php";
...
if (st_check_service_available('st_rental') AND function_exists('st_reg_shortcode')) {
    st_reg_shortcode('ars_rental_price_book', 'ars_vc_rental_price_book');
}