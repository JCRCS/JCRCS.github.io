 ...
 *
 */
if (!class_exists('ARSTransport')) {

    class ARSTransport extends TravelerObject {

        static $_inst;
        protected $post_type = "transport";
        protected $orderby;
         ...
         * @since 1.1.7
         */
        protected $template_folder = 'transports';

        function __construct($transports_id = false) {
            $this->orderby = array(
                'new' => array(
                    'key' => 'new',
        ...
        function init() {
            ...
            add_filter('ars_transport_detail_layout', array($this, 'custom_tour_layout'));

            // add to cart
            add_action('wp_loaded', array($this, 'transports_add_to_cart'), 20);

            //custom search transports template
            add_filter('template_include', array($this, 'choose_search_template'));

            //Sidebar Pos for SEARCH
            add_filter('ars_transport_sidebar', array($this, 'change_sidebar'));

            //Save transports Review Stats
            add_action('comment_post', array($this, '_save_review_stats'));

            // Change transports review arg
            add_filter('ars_transport_wp_review_form_args', array($this, 'comment_args'), 10, 2);
            ...
            add_action('st_wc_cart_item_information_ars_transport', array($this, '_show_wc_cart_item_information'));
            add_action('st_wc_cart_item_information_btn_ars_transport', array($this, '_show_wc_cart_item_information_btn'));
            add_action('st_before_cart_item_ars_transport', array($this, '_show_wc_cart_post_type_icon'));


            add_filter('st_add_to_cart_item_ars_transport', array($this, '_deposit_calculator'), 10, 2);
            if (is_singular('ars_transport')) {
        ...
        function _save_review_stats($comment_id) {
            ...
            if (get_post_type($post_id) == 'ars_transport') {
        ...
        function _show_wc_cart_item_information($st_booking_data = array()) {
            echo st()->load_template('transports/wc_cart_item_information', false, array('st_booking_data' => $st_booking_data));
        }
        ...
        function _st_data_custom_price() {
            return array('title' => 'Price Custom Settings', 'post_type' => 'ars_transport');
        }
        ...
        function _change_preload_search_title($return) {
            if (get_query_var('post_type') == 'transport') {
                $return = __(" Transfers and Transport in %s", ST_TEXTDOMAIN);

                if (STInput::get('desination_id')) {
                    $return = sprintf($return, get_the_title(STInput::get('desination_id')));
                } elseif (STInput::get('destination_name')) {
                    $return = sprintf($return, STInput::get('destination_name'));
...
//                                and mt10.post_type = 'ars_transport'
//                                )";
        ...
        function _get_join_query($join) {
            if (!TravelHelper::checkTableDuplicate('ars_transport'))
                return $join;

            global $wpdb;

            $table = $wpdb->prefix . 'ars_transport';
        ...
        function _get_where_query_tab_location($where) {
            ...
            if (!empty($location_id)) {
                ...
                $where = TravelHelper::_st_get_where_location($location_id, array('ars_transport'), $where);
            }
            return $where;
        }
        ...
        function get_unavailable_tour($check_in, $check_out = '', $people = 1) {
            ...
            $query = "SELECT
					post_id,
					{$wpdb->prefix}ars_transport.max_people,
				...
				FROM
					{$wpdb->prefix}ars_transport
				JOIN {$wpdb->prefix}st_order_item_meta ON {$wpdb->prefix}ars_transport.post_id = {$wpdb->prefix}st_order_item_meta.st_booking_id
				AND {$wpdb->prefix}st_order_item_meta.st_booking_post_type = 'ars_transport'
				WHERE
					1 = 1
				AND
					(
				...
				GROUP BY post_id
				HAVING  {$wpdb->prefix}ars_transport.max_people - total_booked <{$people}
				LIMIT 0,500";
        ...
        function choose_search_template($template) {
            global $wp_query;
            $post_type = get_query_var('post_type');
            if ($wp_query->is_search && $post_type == 'ars_transport') {
        ...
        function get_result_string() {
            ...
            if ($query->found_posts) {
                if ($query->found_posts > 1) {
                    $p1 .= esc_html($query->found_posts) . __(' transports ', ST_TEXTDOMAIN);
                } else {
        ...
        function transports_add_to_cart() {
            if (STInput::request('action') == 'transports_add_to_cart') {
        ...
        function do_add_to_cart() {
            ...
            $item_id = STInput::request('item_id', '');
            if ($item_id <= 0 || get_post_type($item_id) != 'ars_transport') {
            ...
            $booking_period = intval(get_post_meta($item_id, 'transports_booking_period', true));
            $period = STDate::dateDiff($today, $check_in);
        ...
        static function geExtraPrice($extra_price = array()) {
            $total_price = 0;
        ...
        function get_cart_item_html($item_id = false) {
            return st()->load_template('transports/cart_item_html', null, array('item_id' => $item_id));
        }
        ...
        static function get_taxonomy_and_id_term_tour() {
            $list_taxonomy = st_list_taxonomy('ars_transport');
            $list_id_vc = array();
            $param = array();
            $list_value = array();
